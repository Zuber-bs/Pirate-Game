...
function setup() {
  ...
  boat = new Boat(width, height - 100, 200, 200, -100, animation);

  var frames = spritedata.frames;

  for(var i = 0; i < frames.length; i++) {
    var frame = frames[i].position;
    var image = spritesheet.get(frame.x, frame.y, frame.w, frame.h);
...
function draw() {
  ...
  image(animation[frameCount % animation.length], 200, 200, 50, 50);
...
function showBoats() {
  if (boats.length > 0) {
    if (
      ...
      var boat = new Boat(width,height - 100, 200, 200, position, animation);
      boats.push(boat);
    }
    ...
    var boat = new Boat(width, height - 100, 200, 200, -100, animation);
    boats.push(boat);
  }
}
